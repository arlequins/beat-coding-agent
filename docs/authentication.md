...
Production sign-in uses Beat as the OIDC provider. Register a dedicated public
client for Beat Coding Agent; do not reuse the Beat Agent client because each
application has its own callback and audience boundary. Keep the actual issuer,
client ID, API audience, and callback origins in the protected deployment
configuration rather than committing them here.

The production values must follow this shape:

```dotenv
NEXT_PUBLIC_OIDC_AUTHORITY=https://<beat-issuer>/auth
NEXT_PUBLIC_OIDC_CLIENT_ID=<beat-coding-agent-client>
OIDC_ISSUER_URL=https://<beat-issuer>/auth
OIDC_AUDIENCE=<beat-coding-agent-api-audience>
```

The browser client uses Authorization Code + PKCE and has no client secret.

## Environment

```dotenv
# API resource server
OIDC_ISSUER_URL=https://idp.coding.beat.localhost
OIDC_AUDIENCE=example-api
OIDC_ALLOWED_ALGORITHMS=RS256
# OIDC_JWKS_URI=https://idp.coding.beat.localhost/.well-known/jwks.json

# Static browser client
NEXT_PUBLIC_OIDC_AUTHORITY=https://idp.coding.beat.localhost
NEXT_PUBLIC_OIDC_CLIENT_ID=example-spa
NEXT_PUBLIC_OIDC_RESOURCE=https://api.coding.beat.localhost
