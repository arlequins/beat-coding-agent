...
export function AgentChat() {
  ...
  const workspaces = useQuery({
    ...trpc.agent.workspaces.queryOptions(),
  ...
  if (!user)
    return (
      <div className="rounded-lg border border-dashed p-8 text-center">
        <h2 className="text-lg font-semibold">Beat 로그인 후 시작하세요</h2>
        <p className="text-muted-foreground mt-2 text-sm">
          상담 에이전트와 분리된 코딩 workspace가 로그인 후 준비됩니다.
        </p>
      </div>
    );
    ...
    return (
      <form className="rounded-xl border p-6" onSubmit={submitWorkspace}>
        ...
        <p className="text-muted-foreground mt-1 text-sm">
          코드 대화와 문서는 이 코딩 워크스페이스 안에서만 공유됩니다.
        </p>
        <div className="mt-4 flex gap-2">
          <Input
            aria-label="워크스페이스 이름"
            onChange={(event) => setWorkspaceName(event.target.value)}
            placeholder="예: beat-agent 개선"
            value={workspaceName}
          />
        ...
        <details className="mt-5 border-t pt-4">
          ...
          <form className="mt-3 space-y-2" onSubmit={submitDocument}>
            ...
            <Textarea
              ...
              placeholder="코드나 개발 문서를 붙여 넣으면 이 워크스페이스에서 검색합니다."
