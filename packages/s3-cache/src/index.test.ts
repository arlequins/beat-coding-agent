...
describe("createS3Cache", () => {
  it("stores hashed keys and expires values", async () => {
    ...
    expect(await cache.get("user@coding.beat.localhost")).toEqual({ id: 1 });

    now = 11_001;
    expect(await cache.get("user@coding.beat.localhost")).toBeUndefined();
  });
