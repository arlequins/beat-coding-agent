...
describe("template:init", () => {
  ...
  it("supports a minimal preset and independently selected features", async () => {
    ...
    const siteConfig = transformContent(
      ...
      'export const siteConfig = { name: "Beat Coding Agent", shortName: "AW" };\n',
      {
  ...
  it("previews without writing and then initializes tracked text files", async () => {
    ...
    await writeFile(
      packagePath,
      '{"name":"beat-coding-agent","dependencies":{"@arlequins/env":"workspace:*"}}\n',
    );
    await writeFile(
      sstPath,
      'name: "web"; // coding.beat.localhost @arlequins/web\n',
    );
    ...
    assert.match(await readFile(packagePath, "utf8"), /beat-coding-agent/);
